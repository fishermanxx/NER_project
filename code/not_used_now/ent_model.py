...
class BLSTM_CRF(nn.Module):
    ...
    def build_model(self):
        ...
        self.lstm = nn.LSTM(self.embedding_dim, self.hidden_dim//2, batch_first=True, num_layers=1, bidirectional=True)
    ...
    def _get_lstm_features(self, x, use_cuda=None):
        ...
        ##LSTM layer
        if use_cuda:
            h_0 = torch.randn(2, batch_size, self.hidden_dim//2).cuda()  #(n_layer*n_dir, N, n_hid)
        else:
    ...
    def train_model(self, data_loader: KGDataLoader, data_set=None, hyper_param={}, use_cuda=None):
        '''
        :param
        ...
        if data_set is None:
            train_dataset = data_loader.dataset.train_dataset
        else:
        ...
        for epoch in range(EPOCH):
            ...
            for cnt, data_batch in enumerate(data_generator):
                ...
                loss += loss_avg
                if use_cuda:
                    loss_recode.append(loss_avg.cpu().item())
                else:
                    loss_recode.append(loss_avg.item())
    ...
    def eval_model(self, data_loader: KGDataLoader, data_set=None, hyper_param={}, use_cuda=False):
        ...
        if data_set is None:
            eva_data_set = data_loader.dataset.dev_dataset
        else:
            eva_data_set = data_set
